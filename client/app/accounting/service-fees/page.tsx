...
export default function ServiceFeesPage() {
    ...
    const handleMarkPaid = async (id: string, amount: number) => {
        if (!confirm('Á¢∫Ë?Ê®ôË??∫Â∑≤?∂Ê¨æ?')) return;
        try {
    ...
    return (
        <StandardPageLayout title="ÊØèÊ??çÂ?Ë≤ªÁÆ°??(Service Fees)" showBack onBack={() => window.location.href = '/portal'}>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-slate-700">Â∏≥Â??à‰ªΩ:</span>
                        <input
                            type="month"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    <div className="text-sm text-slate-500 mb-1">?¨Ê??âÊî∂Á∏ΩÈ?</div>
                    <div className="text-2xl font-bold text-slate-900">${stats.total.toLocaleString()}</div>
                </div>
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    <div className="text-sm text-slate-500 mb-1">Â∑≤Êî∂?ëÈ?</div>
                    <div className="text-2xl font-bold text-emerald-600">${stats.paid.toLocaleString()}</div>
                </div>
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    <div className="text-sm text-slate-500 mb-1">?™Êî∂È§òÈ?</div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <div className="text-center py-12 text-slate-500">?¨Ê?Â∞öÁÑ°?çÂ?Ë≤ªÂ∏≥??/div>
                ) : (
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-200">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-sm font-medium text-slate-500">?ÖÁõÆ</th>
                                <th className="px-6 py-3 text-sm font-medium text-slate-500">?ëÈ?</th>
                                ...
                                <tr key={r.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        `}>
                                            ...
                                            {r.status === 'PAID' ? 'Â∑≤Êî∂Ê¨? : r.status === 'PENDING' ? '?™Êî∂Ê¨? : r.status}
                                        </span>
                                    </td>
