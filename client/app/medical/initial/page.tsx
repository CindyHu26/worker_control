...
export default function InitialCheckupPage() {
    ...
    return (
        <StandardPageLayout title="?ùÊ¨°È´îÊ™¢ÁÆ°Á? (Initial Checkup)" showBack onBack={() => window.location.href = '/portal'}>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden min-h-[400px]">
                {loading ? (
                    ...
                    /* Pending Table */
                    pendingEntries.length === 0 ? (
                        <div className="text-center py-12 text-slate-500">?°Â??íÁ??ÖÁõÆ</div>
                    ) : (
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 border-b border-slate-200">
                                <tr>
                                    <th className="px-6 py-3 text-sm font-medium text-slate-500">ÁßªÂ∑•ÂßìÂ?</th>
                                    <th className="px-6 py-3 text-sm font-medium text-slate-500">?á‰∏ª</th>
                                    <th className="px-6 py-3 text-sm font-medium text-slate-500">?•Â??•Ê?</th>
                                    <th className="px-6 py-3 text-sm font-medium text-slate-500">?âÈ?Ê™¢Ê???(3?•ÂÖß)</th>
                                    <th className="px-6 py-3 text-center text-sm font-medium text-slate-500">?ç‰?</th>
                                </tr>
                            </thead>
                    ...
                    scheduledChecks.length === 0 ? (
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 border-b border-slate-200">
                                <tr>
                                    <th className="px-6 py-3 text-sm font-medium text-slate-500">ÁßªÂ∑•ÂßìÂ?</th>
                                    <th className="px-6 py-3 text-sm font-medium text-slate-500">ÂÆâÊ??•Ê?</th>
                                    <th className="px-6 py-3 text-sm font-medium text-slate-500">?´Èô¢</th>
                                    <th className="px-6 py-3 text-sm font-medium text-slate-500">?Ä??/th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                    ...
                                    <tr key={check.id} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            `}>
                                                ...
                                                {check.result === 'pending' ? 'Á≠âÂ?Ê™¢Êü•/?±Â?' : check.result}
                                            </span>
                                        </td>
                                    </tr>
                                ))}
            ...
            {showScheduleModal && selectedDeployment && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
                        <h3 className="text-xl font-bold text-slate-800 mb-4">ÂÆâÊ??ùÊ¨°È´îÊ™¢</h3>

                        <div className="space-y-4 mb-6">
                            <div className="bg-slate-50 p-3 rounded-lg text-sm">
                                <p><span className="text-slate-500">ÁßªÂ∑•:</span> {selectedDeployment.worker.englishName}</p>
                                <p><span className="text-slate-500">?•Â???</span> {new Date(selectedDeployment.entryDate).toLocaleDateString()}</p>
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">È´îÊ™¢?•Ê?</label>
                                <input
                                    type="date"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">?´Èô¢</label>
                                <input
                                    type="text"
                                    placeholder="‰æãÂ?: Ë°õÁ?Á¶èÂà©?®Ê??íÈÜ´??
                                    className="w-full border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500"
                                    value={hospitalName}
